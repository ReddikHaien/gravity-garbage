use std::{collections::{HashSet, HashMap, VecDeque}, thread, sync::mpsc::{self, Sender}, time::Instant, mem::swap};
...
impl Collector{
    ...
    fn run(&mut self){
        ...
        max = self.objects.iter().map(|x|unsafe{x.as_ref().unwrap().position}).max().unwrap_or(0);
